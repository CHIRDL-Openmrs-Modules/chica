...
public class pageByClinic implements Rule {
	...
	public Result eval(LogicContext context, Integer patientId, Map<String, Object> parameters) throws LogicException {
		...
		if (locationId == null) {
			log.error("Location ID parameter not found.  No one will be paged.");
			return Result.emptyResult();
		} else if (locationAttr == null) {
			log.error("Location Attribute Value parameter not found.  No one will be paged.");
			return Result.emptyResult();
		} else if (message == null) {
			log.error("Message parameter not found.  No one will be paged.");
			return Result.emptyResult();
		}
		...
		String pagerMessage = clinicDisplayName + " - " + message;
		
		// Get the pager numbers
		lav = service.getLocationAttributeValue(locationId, locationAttr);
		if (lav == null || lav.getValue() == null || lav.getValue().trim().length() == 0) {
		...
		while (tokenizer.hasMoreTokens()) {
			...
			if (response.contains("send failed")) {
                ...
                if (response.contains("send failed")) {
                	...
                	Error error = new Error("Info", locationAttr, "Support page sent: " + message, null,
                		new java.util.Date(), null);
    				chirdlutilbackportsService.saveError(error);
                }
			} else {
