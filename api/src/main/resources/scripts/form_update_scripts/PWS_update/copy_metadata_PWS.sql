...
update field f1 
join (select * from field where field_id in (select field_id from form_field where form_id in (select form_id from form where name='PWS' and retired=0))) f2
on f1.name=f2.name
set f1.concept_id=f2.concept_id, f1.default_value=f2.default_value, f1.field_type=f2.field_type
where f1.field_id in (select field_id from form_field where form_id in (select form_id from form where name='PWS_new'));

/*copy field_number to the new form*/
update form_field f1,
(select b.name,a.* from (select * from form_field where form_id in (select form_id from form where name='PWS' and retired=0)) a
inner join field b
on a.field_id=b.field_id
) f2,
(select b.name,a.* from (select * from form_field where form_id in (select form_id from form where name='PWS_new')) a
inner join field b
on a.field_id=b.field_id
) f3 
...
(select field_id as child_field_id, parent_form_field from form_field where form_id in (select form_id from form where name='PWS' and retired=0)) a
...
select form_field_id,field_id from form_field where form_id in (select form_id from form where name='PWS_new')) a
